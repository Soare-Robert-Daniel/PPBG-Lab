...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    if (texture2 != nullptr) {
        glUniform1i(glGetUniformLocation(shader->program, "has_second_texture"), 1);
    }
    else {
        glUniform1i(glGetUniformLocation(shader->program, "has_second_texture"), 0);
    }
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    for (int i = 0; i < size; i++) {
        data[i] = (unsigned char)( glm::sin(  Engine::GetElapsedTime() * i ) * glm::cos(i) * 256 );
    }
