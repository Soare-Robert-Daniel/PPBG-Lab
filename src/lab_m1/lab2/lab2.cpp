...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(1, 0,  1), glm::vec3(0.2, 1, 0.6)),
            VertexFormat(glm::vec3(0, 1,  1), glm::vec3(0, 0.5, 1)),
            VertexFormat(glm::vec3(1, 1,  1), glm::vec3(1, 1, 0.1)),
            VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 0.1, 1)),
            VertexFormat(glm::vec3(1, 0,  0), glm::vec3(1, 0.5, 0)),
            VertexFormat(glm::vec3(0, 1,  0), glm::vec3(0.3, 1, 0.5)),
            VertexFormat(glm::vec3(1, 1,  0), glm::vec3(0.5, 0.5, 0)),
        };
    ...
    // by definition!
    {
        vector<VertexFormat> vertices
        {
            VertexFormat(glm::vec3(0, 0,  1), glm::vec3(0, 0.5, 1)),
            // TODO(student): Complete the vertices data for the cube mesh
            VertexFormat(glm::vec3(1, 0,  1), glm::vec3(0.2, 1, 0.6)),
            VertexFormat(glm::vec3(0, 1,  1), glm::vec3(0, 0.5, 0.5)),
            VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0.8, 1, 0.1)),
        };
        ...
        // Actually create the mesh from the data
        CreateMesh("tetr", vertices, indices);
    }
    ...
    // Create a mesh box using custom data
    {
        ...
        // Actually create the mesh from the data
        CreateMesh("cub_trian", vertices, indices);
    }
    ...
    // BONUS: desenare cerc
    {
        ...
        constexpr int NR_PUNCTE = 100;
        constexpr float r = 1;


        constexpr float totalCerc = glm::pi<float>() * 2;
        constexpr float da = totalCerc / NR_PUNCTE;

        for (float i = 0; i <= NR_PUNCTE; i++) {
            float x = r * glm::cos(i * da);
            float y = r * glm::sin(i * da);

            vertices.push_back(
                VertexFormat(glm::vec3(x, y, 0), glm::vec3(0.5, x / r, y / r))
            );
        }
        ...
        meshes["circle"] = new Mesh("circle bonus");
    ...
    {
        vector<VertexFormat> vertices = {
            VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.5, 0, 0.5))
        };
        ...
        meshes["disc"] = new Mesh("circle bonus");
        meshes["disc"]->InitFromData(vertices, indices);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(-1.5f, 2, 0), glm::vec3(0.5f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetr"], shaders["VertexColor"], glm::vec3(1.5, 2, 0), glm::vec3(0.25f));

    // TODO(student): Draw the square
    RenderMesh(meshes["cub_trian"], shaders["VertexColor"], glm::vec3(0, 2, 0), glm::vec3(0.5f));

    // BONUS: cerc simplu
    RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(1, 1, 0), glm::vec3(0.5f));

    // BONUS: disc simplu
    RenderMesh(meshes["disc"], shaders["VertexColor"], glm::vec3(-1, 1, 0), glm::vec3(0.3f));
